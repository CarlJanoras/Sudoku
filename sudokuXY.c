...
int inY(int candidate, int noOfStacks, int stackSize, int stackOptions[noOfStacks][stackSize], int topOfStacks[noOfStacks], int currentRow, int currentCol) {
  int row, col, gridValue, currentStack, gridSize = stackSize-1;
...
void puzzleSolutions(int** grid, int gridSize, int noOfStacks, int subgridSize) {
  ...
  for (row = 0; row < gridSize; row++) {
    if (solvable == 0) {
      for (col = 0; col < gridSize; col ++) {
        if (((row == col && row <= gridSize/2 && col <= gridSize/2) || (row+col== gridSize-1 && row <= gridSize/2 && col >= gridSize/2) || (row >= gridSize/2 && col == gridSize/2)) && (row == col || row+col == gridSize-1)) { //check the initial values in the 'Y' of the grid
          currentStack = (row*gridSize)+col; //computes the index of the currentStack
