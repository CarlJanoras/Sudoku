...
public class Sudoku_UI{
  ...
  //variable for storing solutions for the most recently solved puzzle
  List solutionsList;
  ...
  Sudoku_UI(int nPuzzles,final int [] subgrids, final LinkedList <int [][]> initPuzzles) throws Exception{
    ...
    checkRegular.addActionListener(new ActionListener(){
      public void actionPerformed(ActionEvent e){
        int[][] puzzle = checkInput(jtf);
        int c1;
        int c2, c3;
        int sub = puzzle.length;
        boolean error = false;
        sub = (int) Math.sqrt(sub);
        for(int j=0; j < puzzle.length; j++){
          int [] line1 = new int[puzzle.length];
          int [] line2 = new int[puzzle.length];
          int [] line3 = new int[puzzle.length];
          for(int k=0; k < puzzle.length; k++){
            line1[k] = puzzle[j][k];
            line2[k] = puzzle[k][j];
            //line3[j] = puzzle[(i / sub) * sub + j / sub][i * sub % puzzle.length + j % sub];
          }
          c1 = checkLine(line1);
          c2 = checkLine(line2);
          //c3 = checkLine(line2);
          if(c1 < puzzle.length && c2 < puzzle.length /*&& c3 < puzzle.length*/){
            jtf[j][c1].setBackground(Color.RED);
            jtf[j][c2].setBackground(Color.RED);
            //jtf[j][c3].setBackground(Color.RED);
            error = true;
          }
        }
        if(error == false){
          JOptionPane.showMessageDialog(frame,"<3 <3 YOU DID IT! <3 <3", "CONGRATULATIONS!",    JOptionPane.PLAIN_MESSAGE);
        }
        System.out.println();
      }
    });
...
<<<<<<< HEAD
  ...
  int [][] checkInput(JTextField [][] jtf){
    ...
    for(int j=0; j < puzzle.length; j++){
      System.out.println();
      for(int k=0; k < puzzle.length; k++){
        if(jtf[j][k].getText().equals("")){ //if field is empty
          jtf[j][k].setBackground(Color.RED);
          error = true;
        }else if(Integer.parseInt(jtf[j][k].getText()) > 0 && Integer.parseInt(jtf[j][k].getText()) < puzzle.length+1){
          puzzle [j][k] = Integer.parseInt(jtf[j][k].getText());
          jtf[j][k].setBackground(Color.WHITE);
    ...
    if(error){
      JOptionPane.showMessageDialog(frame,"Required input!", "Error",    JOptionPane.PLAIN_MESSAGE);
    }
    return puzzle;
  }
  
  int [][] getInput(JTextField [][] jtf){
    int [][] puzzle = new int[jtf.length][jtf.length];
    boolean error = false;
    for(int j=0; j < puzzle.length; j++){
      System.out.println();
  ...
  //for checking Regular puzzle
  int checkLine(int [] line){
    int [] checker = new int[line.length+1];
    int index = line.length;
    
    for(int i = 0; i < line.length; i++){
      if(checker[line[i]] == 0){
        checker[line[i]] = line[i];
      }else{
        index = i;
        break;
      }
